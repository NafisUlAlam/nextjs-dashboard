...
const errorTextClass = "mt-2 text-sm text-red-500";
  ...
  return (
    <form action={formAction}>
      <div className="rounded-md bg-gray-50 p-4 md:p-6">
          ...
          <div id="customer-error" aria-atomic="true" aria-live="polite">
            {state.errors?.customerId &&
              state.errors.customerId.map((error: string) => (
                <p className={errorTextClass} key={error}>
                  {error}
                </p>
              ))}
          </div>
          ...
          <div id="amount-error" aria-live="polite" aria-atomic="true">
            {state.errors?.amount &&
              state.errors.amount.map((error: string) => {
                return (
                  <p key={error} className={errorTextClass}>
                    {error}
                  </p>
                );
              })}
        ...
        {state.message && (
          <div>
            <p className={errorTextClass}>{state.message}</p>
          </div>
        )}
      </div>
